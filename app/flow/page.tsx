...
const frontendSteps = [
  ["Upload", "User selects card image"],
  ["Check", "Validate file type"],
  ["Preview", "Show image + progress"],
  ["Encode", "Convert to base64"],
  ["Send", "POST to /api/recognize"],
];

const backendSteps = [
  ["Receive", "Next.js API route"],
  ["Validate", "API key + image type"],
  ["Prompt", "Image + JSON schema"],
  ["Clean", "Parse LLM JSON"],
  ["Return", "Contact + raw text"],
];
        ...
        <header className="flex flex-col gap-5 border-b border-slate-200 pb-6 md:flex-row md:items-end md:justify-between">
          <div className="max-w-3xl">
            ...
            <p className="mt-4 max-w-2xl text-lg leading-8 text-slate-600">
              The browser prepares the image. The backend asks the vision LLM.
              The user reviews the result before saving locally.
            </p>
          </div>
          <Link
            href="/"
          ...
          <section className="flex flex-col gap-5">
            <section className="rounded-lg border border-violet-200 bg-white/80 p-4 shadow-sm">
              <div className="mb-4 flex items-center gap-3">
                ...
                <div>
                  <h2 className="text-2xl font-semibold">Vision LLM</h2>
                  <p className="text-sm font-medium text-slate-500">
                    OpenRouter model
                  </p>
                </div>
        ...
        <section className="grid gap-4 rounded-lg border border-red-200 bg-red-50 px-5 py-4 text-red-950 shadow-sm md:grid-cols-[auto_1fr] md:items-center">
          ...
          <div>
            ...
            <p className="text-base leading-7">
              Invalid file, API failure, or LLM parse failure returns an error.
              The frontend shows a toast and resets the scan state.
            </p>
          </div>
        </section>
      </div>
